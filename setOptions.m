...
paths.env.resultsDir = ['..',filesep,'data',filesep,'results',filesep]; % path results files
...
options.model.obs_config  = {'tapas_unitsq_sgm_config_mod','tapas_unitsq_sgm_config_mod'};
...
options.model.sound   = readmatrix([paths.env.workingDir,filesep,'sound.csv']);
options.model.outcome = readmatrix([paths.env.workingDir,filesep,'outcome.csv']);

% optimization algorithm
addpath(genpath(paths.env.modelDir));
savepath((paths.env.modelDir));

options.model.nRuns = 6; %softcode!
if options.model.nRuns>1
    for iRuns = 1:options.model.nRuns
        paths.env(iRuns).simulationsDir = ['..',filesep,'data',filesep,'sim',filesep,'run',num2str(iRuns),filesep];
    end
else
paths.env.simulationsDir = ['..',filesep,'data',filesep,'sim',filesep]; % path results files
end
...
for i = 1:options.dataSet.nParticipants
    options.participant(i,1).rawdir     = [paths.env.data,d(i).name];
    if options.model.nRuns >1
        for r = 1:options.model.nRuns
        options.participant(i,r).resultsdir = [paths.env.resultsDir,'run',num2str(r),filesep,d(i).name];
        options.participant(i,r).dataFile = [options.participant(i,r).rawdir,filesep,'behav.mat'];
        end
    else
    options.participant(i,1).resultsdir = [paths.env.resultsDir,d(i).name];
    options.participant(i,1).dataFile = [options.participant(i,r).rawdir,filesep,'behav.mat'];
